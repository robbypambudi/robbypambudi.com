...
const buttonVariants = cva(
  'ui-button inline-flex items-center justify-center whitespace-nowrap rounded-base text-base md:text-sm font-medium ring-offset-white transition-all gap-2 [&_svg]:pointer-events-none [&_svg]:size-5 md:[&_svg]:size-4 [&_svg]:shrink-0 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 active:scale-[0.98]',
  {
    variants: {
      ...
      size: {
        default: 'h-12 md:h-10 px-5 md:px-6 py-2',
        sm: 'h-11 md:h-9 px-4 md:px-3',
        lg: 'h-12 md:h-11 px-6 md:px-8',
        icon: 'size-11 md:size-10',
        xl: 'h-12 sm:h-14 md:h-12 px-5 sm:px-8 md:px-12 w-full sm:w-auto',
      },
    },
    defaultVariants: {
      variant: 'default',
